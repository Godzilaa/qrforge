...
import { ArrowLeft, Download, Loader2, Save, Lock } from "lucide-react";
...
export default function QRDetail() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/qr/${numId}/analytics`).then((r) => r.json()),
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/qr/${qr.id}`, {
        ...
        body: JSON.stringify({ destinationUrl: newDest, label: newLabel }),
      });
  ...
  const maxTimeline = analytics ? Math.max(...analytics.timeline.map((t) => t.count), 1) : 1;
  const maxDevice = analytics ? Math.max(...analytics.devices.map((d) => d.value), 1) : 1;
  const maxBrowser = analytics ? Math.max(...analytics.browsers.map((b) => b.value), 1) : 1;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6">
          ...
          <div className="space-y-4">
            ...
            <div className="border border-[#0d0d0d] bg-[#030303] p-4 space-y-3">
              ...
              {qr.type === "dynamic" && (
                <div className="space-y-1">
                  <label className="text-[#333] text-[10px]">Destination URL</label>
                  <input
                    value={newDest}
          ...
          <div className="space-y-4">
            {!isPro && (
              <div className="border border-[#FFB800]/20 bg-[#FFB800]/5 p-4 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Lock size={12} className="text-[#FFB800]" />
                  <span className="text-[#FFB800] text-xs">Full analytics available on Pro plan</span>
                </div>
                <Link to="/pricing">
                  <button className="text-[10px] bg-[#FFB800] text-black font-bold px-3 py-1 hover:bg-[#e6a700] transition-colors">
                    UPGRADE
                  </button>
                </Link>
              </div>
